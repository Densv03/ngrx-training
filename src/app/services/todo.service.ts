...
export class TodoService {
  private todosSubject: BehaviorSubject<TodoItem[]> = new BehaviorSubject<
    TodoItem[]
  >(initialState);

  private activeTodoSubject: BehaviorSubject<TodoItem | null> =
    new BehaviorSubject<TodoItem | null>(null);
  ...
  public findTodoById$(id: number): Observable<TodoItem | null> {
    const foundTodo = this.todosSubject
      .getValue()
  ...
  public deleteTask$(id: number): Observable<TodoItem[]> {
    this.todosSubject.next(
      this.todosSubject.getValue().filter((item) => item.id !== id)
    );
    return this.todoList$;
  }
  ...
  public updateActiveTask$(
    ...
    const tasksArr = this.todosSubject
      .getValue()
      .map((todo) => (todo.id === activeTodo?.id ? activeTodo : todo));
