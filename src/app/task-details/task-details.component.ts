...
export class TaskDetailsComponent implements OnInit {
  ...
  public ngOnInit() {
    this.route.url.pipe(map((url) => url[1].path)).subscribe((id) => {
      this.store.dispatch(getActiveTask({ id: parseInt(id) }));
    });

    this.activeTodo$
      .pipe(filter((todoItem) => todoItem !== null))
      .subscribe((todoItem: TodoItem | null) =>
        this.form.patchValue({ ...todoItem })
      );
  }

  public saveChanges(
    todoItem: { name: string; isCompleted: boolean },
    id: number
  ): void {
    this.store.dispatch(
      updateActiveTask({
